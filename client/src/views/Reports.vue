...
const loadData = async () => {
  ...
  try {
    ...
    error.value = t('reports.error.loadFailed') + ': ' + err.message
  } finally {
    loading.value = false
  }
}
...
const getChangeValue = (current, previous) => {
  ...
  if (change > 0) return '+' + formatCurrencyWithDecimals(change, currentCurrency.value, 2)
  if (change < 0) return '-' + formatCurrencyWithDecimals(Math.abs(change), currentCurrency.value, 2)
  return formatCurrencyWithDecimals(0, currentCurrency.value, 2)
}
